...
export default function Home() {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      toast.error("Erro ao criar o atendimento!");
  ...
  const eventOptions: EventOption = {
    ...
    APPOINTMENT_ACCEPTED: "Atendimento aceito pelo médico",
    ...
    PATIENT_VIDEO_NOSHOW: "No-show realizado pelo paciente (médico não entrou)",
    DOCTOR_VIDEO_NOSHOW: "No-show realizado pelo médico (paciente não entrou)",
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100 text-black">
      ...
      >
        <label className="block mb-2">
          Event Type (Status do atendimento):
          <select
            {...register("eventType")}
        ...
        <label className="block mb-2">
          Appointment: (ID único do atendimento)
          <input
            {...register("resource.appointment")}
            type="number"
            className="w-full p-2 border rounded mt-1"
          />
        </label>
